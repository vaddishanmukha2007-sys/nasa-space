...
const HistoryItemCard: React.FC<{ item: ClassificationHistoryItem }> = ({ item }) => {
  ...
  const formattedData = {
      orbitalPeriod: {
          value: convertToDisplayValue(item.data.orbitalPeriod, units.orbitalPeriod).toFixed(2),
          unit: units.orbitalPeriod === 'days' ? 'd' : 'y',
      },
      planetaryRadius: {
          value: convertToDisplayValue(item.data.planetaryRadius, units.planetaryRadius).toFixed(2),
          unit: units.planetaryRadius === 'earth' ? 'R⊕' : 'RJ',
      },
      stellarTemperature: {
          value: convertToDisplayValue(item.data.stellarTemperature, units.stellarTemperature).toFixed(0),
    ...
    <main className="mt-8 animate-fade-in">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-amber-500/20 rounded-lg shadow-lg p-6 text-center">
             <div className="relative w-32 h-32 mx-auto mb-4 group">
              <img 
                src={avatar} 
                alt="User Avatar" 
                className="w-full h-full rounded-full border-4 border-amber-500/50 bg-slate-200 dark:bg-slate-700 object-cover" 
              />
              <div 
            ...
            {!isEditing ? (
                <>
                    ...
                    <p className="text-slate-500 dark:text-gray-400">guest@exoplanet-detector.ai</p>
                    <p className="text-xs text-slate-400 dark:text-gray-500 mt-2">Member Since: {joinDate}</p>
                    <button 
                        onClick={handleEdit}
                        className="mt-6 w-full bg-slate-500 hover:bg-slate-600 dark:bg-slate-600 dark:hover:bg-slate-500 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-slate-400 shadow-lg">
                        Edit Profile
                    </button>
                </>
            ) : (
                <div className="space-y-4 text-left">
                    <div>
                        ...
                        <input 
                            ...
                            className="shadow-inner appearance-none border rounded-lg w-full py-2 px-3 text-slate-800 dark:text-gray-200 bg-slate-100 dark:bg-slate-900/60 border-slate-400 dark:border-slate-600 leading-tight focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all duration-300"
                        />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-slate-500 dark:text-gray-400 mb-1">Email</label>
                        <p className="text-slate-400 dark:text-gray-500 text-sm py-2 px-3">guest@exoplanet-detector.ai</p>
                    </div>
                    <div>
                        <label htmlFor="joinDate" className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">Member Since</label>
                        <input 
                            type="text" 
                            id="joinDate" 
                            value={editJoinDate}
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button
                            ...
                            className="w-full bg-slate-500 hover:bg-slate-600 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300">
                            Cancel
                        </button>
                    </div>
                </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-slate-800/50 border border-slate-300 dark:border-amber-500/20 rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-bold mb-4 text-amber-500 dark:text-amber-400">Classification History</h2>
            {history.length === 0 ? (
              <div className="text-center py-16 animate-fade-in">
                <p className="text-slate-500 dark:text-gray-400">No classifications recorded yet.</p>
                <p className="text-sm text-slate-400 dark:text-gray-500 mt-2">Classify a candidate on the dashboard to see your history.</p>
              </div>
            ) : (
